...
class MusicImporter:
    ...
    def import_track_from_spotify(self, spotify_track_id: str, db: Session, skip_mb_lookup: bool = False):
        ...
        if primary_artist_id:
            same_title_songs = (
                ...
                .filter(models.Song.title == track_data["name"], models.Song.id != new_song.id)
                .all()
            )
            ...
            for s in same_title_songs:
                ...
                first_artist_link = (
                    ...
                    .filter(models.SongArtistLink.song_id == s.id, models.SongArtistLink.role_category == "Artist")
                    .first()
                )
        ...
        for i, album in enumerate(albums):
            ...
            if not db_album:
                ...
                db_album_group = (
                    ...
                    .filter(models.AlbumGroup.title == album["name"], models.AlbumGroup.artist_id == artist_id)
                    .first()
                )
            ...
            for j, track in enumerate(tracks):
                ...
                if progress_callback:
                    progress_callback(track_prog, f"[{album['name']}] Importing track {j+1}/{total_tracks}")
                try:
                    song = self.import_track_from_spotify(track["id"], db, skip_mb_lookup=True)
                    imported_tracks += 1

                    # AlbumTrack を作成 (duration_ms 含む)
                    existing_album_track = (
                        db.query(models.AlbumTrack)
                        .filter(models.AlbumTrack.album_id == db_album.id, models.AlbumTrack.song_id == song.id)
                        .first()
                    )
    ...
    def import_album_bulk(self, spotify_album_id: str, db: Session, progress_callback=None) -> dict:
        ...
        if progress_callback:
            progress_callback(10, "Fetching album info...")
        album = self.spotify.get_album(spotify_album_id)

        if not album:
            if progress_callback:
                progress_callback(95, "Album not found. Finalizing import...")
            return {"imported_albums": 0, "imported_tracks": 0}

        if progress_callback:
            progress_callback(20, f"Processing album: {album['name']}")
    ...
    def import_playlist_bulk(self, spotify_playlist_id: str, db: Session, progress_callback=None) -> dict:
        ...
        if progress_callback:
            progress_callback(5, "Fetching playlist tracks...")
        ...
        if progress_callback:
            progress_callback(10, f"Found {total_artists} unique artists in playlist. Starting sync...")

        for i, artist_id in enumerate(artist_ids_list):
            base_prog = 10 + int(80 * ((i + 1) / total_artists))
            if progress_callback:
                progress_callback(base_prog, f"Syncing artist {i+1}/{total_artists}...")
        ...
        if progress_callback:
            progress_callback(95, "Finalizing playlist sync...")
        return {
    ...
    def import_mb_releases_bulk(self, release_ids: list[str], db: Session, progress_callback=None) -> dict:
        ...
        for i, release_id in enumerate(release_ids):
            if progress_callback:
                progress_callback(int(i / total_releases * 100), f"Fetching release {i+1}/{total_releases}")
            ...
            db_album_group = (
                db.query(models.AlbumGroup)
                .filter(models.AlbumGroup.title == album_title, models.AlbumGroup.artist_id == artist_id)
                .first()
            )

            if not db_album_group:
                db_album_group = models.AlbumGroup(
                    title=album_title, artist_id=artist_id, release_date=parsed_date, album_type="album"
                )
                db.add(db_album_group)
                db.commit()
                db.refresh(db_album_group)
            ...
            for m in media:
                ...
                for track in tracks:
                    ...
                    is_video = (
                        "dvd" in media_format
                        or "blu-ray" in media_format
                        or "video" in media_format
                        or "vhs" in media_format
                    )

                    new_song = models.Song(title=track_title, is_streaming_available=False, is_video=is_video)
                    db.add(new_song)
                    db.commit()
                    db.refresh(new_song)
                    imported_tracks += 1
                    ...
                    same_title_songs = (
                        db.query(models.Song)
                        .filter(models.Song.title == track_title, models.Song.id != new_song.id)
                        .all()
                    )
                    ...
                    for s in same_title_songs:
                        first_artist_link = (
                            db.query(models.SongArtistLink)
                            .filter(
                                models.SongArtistLink.song_id == s.id, models.SongArtistLink.role_category == "Artist"
                            )
                            .first()
                        )
