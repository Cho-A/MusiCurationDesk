...
router = APIRouter(
    prefix="/external", tags=["External APIs"], dependencies=[Depends(dependencies.get_current_admin_user)]
)
...
def import_spotify_track(req: ImportRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db_artist = db.query(models.Artist).filter(func.lower(models.Artist.name) == func.lower(artist_name)).first()
        ...
        db_album_group = (
            db.query(models.AlbumGroup).filter(func.lower(models.AlbumGroup.title) == func.lower(album_name)).first()
        )
        ...
        db_album = db.query(models.Album).filter(func.lower(models.Album.main_title) == func.lower(album_name)).first()
        ...
        db_song = db.query(models.Song).filter(func.lower(models.Song.title) == func.lower(track_name)).first()

        if not db_song:
            db_song = models.Song(
                title=track_name, spotify_song_id=track["id"], is_streaming_available=track.get("is_playable", True)
            )
            db.add(db_song)
            db.commit()
            db.refresh(db_song)
            ...
            # リレーション (Song - Artist)
            link = models.SongArtistLink(song_id=db_song.id, artist_id=db_artist.id, role_category="Artist")
            db.add(link)
            ...
            return {
                "status": "skipped",
                "message": f"'{track_name}' already exists in database.",
                "song_id": db_song.id,
            }
...
def bulk_import_setlists(req: BulkImportRequest, db: Session = Depends(get_db)):
    errors = []
    successes = []
...
def full_sync_artist(req: FullSyncRequest, db: Session = Depends(get_db)):
    try:
...
def import_setlistfm(req: SetlistImportRequest, db: Session = Depends(get_db)):
    try:
