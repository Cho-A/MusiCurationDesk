...
router = APIRouter(
    prefix="/search/external", tags=["External Search"], dependencies=[Depends(dependencies.get_current_admin_user)]
)
...
def background_import_playlist(job_id: str, spotify_playlist_id: str):
    ...
    try:
        ...
        update_job_progress(
            ...
            "completed",
            100,
            f"Imported {result['imported_artists']} artists and {result['imported_tracks']} tracks.",
        )
    except Exception as e:
        update_job_progress(job_id, "failed", 0, f"Error: {str(e)}")
    finally:
...
def search_artists(q: str = Query(..., description="Artist name query"), limit: int = Query(10, le=50)):
    ...
    try:
        ...
        for a in artists:
            ...
            result.append(
                {"spotify_artist_id": a["id"], "name": a["name"], "image_url": image_url, "genres": a["genres"]}
            )
        return result
    except Exception as e:
...
def import_track(spotify_song_id: str, db: Session = Depends(get_db)):
