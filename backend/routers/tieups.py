...
router = APIRouter(prefix="/tieups", tags=["Tieups"])


@router.post("/", response_model=schemas.Tieup)
def create_tieup(tieup: schemas.TieupCreate, db: Session = Depends(get_db)):
    db_tieup = models.Tieup(**tieup.model_dump())
    db.add(db_tieup)
    db.commit()
    db.refresh(db_tieup)
    return db_tieup
...
def get_tieups(db: Session = Depends(get_db)):
    return db.query(models.Tieup).all()


@router.get("/{tieup_id}", response_model=schemas.TieupDetail)
def get_tieup(tieup_id: int, db: Session = Depends(get_db)):
    """
    ...
    while current:
        parents.insert(0, schemas.TieupHierarchyNode(id=current.id, name=current.name, category=current.category))
...
def get_tieup_songs(tieup_id: int, db: Session = Depends(get_db)):
    """
