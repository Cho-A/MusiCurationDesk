...
@router.get("/{performance_id}", response_model=schemas.PerformanceDetail)
...
# ----------------------------------------------------
@router.post("/roster", response_model=schemas.PerformanceRoster)
...
# [GET] /performances/
# ----------------------------------------------------
@router.get("/", response_model=List[schemas.PerformanceSummary])
def get_all_performances(
    # クエリパラメータを定義
    artist_id: int | None = Query(None, description="メインアクトのアーティストIDでフィルタリングします。"),
    skip: int = Query(
        0,
        ge=0,
...
@router.put("/{performance_id}/setlist", response_model=List[schemas.SetlistEntry])
def update_setlist(
    performance_id: int,
...
# ----------------------------------------------------
@router.post("/setlist-entries", response_model=schemas.SetlistEntry)
def create_setlist_entry(
    ...
    # --- 紐付けデータを作成 ---
    new_entry = models.SetlistEntry(**entry.model_dump())
...
@router.put("/{performance_id}", response_model=schemas.PerformanceDetail)
...
@router.post("/{performance_id}/copy-setlist", response_model=schemas.PerformanceDetail)
def copy_setlist(
    performance_id: int,
...
@router.post("/bulk-update-tour")
def bulk_update_tour(req: BulkUpdateTourRequest, db: Session = Depends(get_db)):
...
@router.post("/bulk-copy-setlist")
def bulk_copy_setlist(req: BulkCopySetlistRequest, db: Session = Depends(get_db)):
