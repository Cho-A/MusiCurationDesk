...
router = APIRouter(prefix="/works", tags=["MusicalWorks"])


@router.get("/", response_model=List[schemas.MusicalWork])
def search_works(
    q: str | None = Query(None, description="楽曲名検索キーワード"), limit: int = 50, db: Session = Depends(get_db)
):
    """
...
def create_work(work: schemas.MusicalWorkBase, db: Session = Depends(get_db)):
    """
    新しい楽曲 (MusicalWork) を作成します。
    """
    new_work = models.MusicalWork(title=work.title, jasrac_code=work.jasrac_code, iswc_code=work.iswc_code)
    db.add(new_work)
    try:
        db.commit()
        db.refresh(new_work)
...
def update_work(work_id: int, work: schemas.MusicalWorkBase, db: Session = Depends(get_db)):
    """
    Work情報を更新します。
    """
...
def add_credit_to_work(work_id: int, link: schemas.WorkArtistLinkCreate, db: Session = Depends(get_db)):
    """
    ...
    new_link = models.WorkArtistLink(**link.model_dump())
    db.add(new_link)
