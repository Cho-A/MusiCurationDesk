...
# 登録されたIDを保持するグローバル辞書
ids = {"user": {}, "artist": {}, "song": {}, "tour": {}, "performance": {}, "album": {}, "tieup": {}}
...
def post_data(endpoint: str, data: dict):
    ...
    log_name = (
        ...
        or f"Link/Entry (ID: {data.get('song_id') or data.get('performance_id')})"
    )
...
def insert_initial_data():
    ...
    # 1d. ツアー登録
    ids["tour"]["tour2025"] = post_data("/tours/", {"name": "UNISON SQUARE GARDEN TOUR 2025-2026 「うるわしの前の晩」"})
    ids["tour"]["buzzrhythm"] = post_data("/tours/", {"name": "バズリズム LIVE 2020"})

    # 1d-2. 会場登録
    ids["venue"] = {}
    ids["venue"]["kanamoto"] = post_data(
        "/venues/", {"name": "カナモトホール", "prefecture": "北海道", "capacity": 1500}
    )
    ids["venue"]["air_g"] = post_data("/venues/", {"name": "AIR-G' スタジオ", "prefecture": "北海道"})

    # 1e. 公演登録 (北海道公演)
    ids["performance"]["hokkaido"] = post_data(
        "/performances/",
        {
    ...
    # 1e-2. ラジオイベント (セトリなし)
    ids["performance"]["radio_event"] = post_data(
        "/performances/",
        {
            "artist_id": ids["artist"]["unison"],
    ...
    ids["performance"]["buzz_session"] = post_data(
        "/performances/",
        {
    ...
    ids["tieup"] = {}
    # シリーズ (IP)
    ids["tieup"]["tiger_bunny_ip"] = post_data("/tieups/", {"name": "TIGER & BUNNY", "category": "Series"})
    # アニメ版
    if ids["tieup"]["tiger_bunny_ip"]:
        ids["tieup"]["tiger_bunny_anime"] = post_data(
            "/tieups/", {"name": "TIGER & BUNNY", "category": "Anime", "parent_id": ids["tieup"]["tiger_bunny_ip"]}
        )
    ...
    if ids["song"]["sentipiri"] and ids["artist"]["tabuchi"]:
        ...
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["tabuchi"], "role_category": "Composer"}
        )
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["tabuchi"], "role_category": "Lyricist"}
        )
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["tabuchi"], "role_category": "Bassist"}
        )
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["saito"], "role_category": "Vocalist"}
        )
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["saito"], "role_category": "Guitarist"}
        )
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["suzuki"], "role_category": "Drummer"}
        )
        post_data(
            "/song_artist_links/", {"song_id": s_id, "artist_id": ids["artist"]["unison"], "role_category": "Artist"}
        )
    ...
    if ids["performance"]["hokkaido"] and ids["song"]["sentipiri"] and ids["song"]["orion"]:
        ...
        post_data(
            "/setlist_entries/",
            {"performance_id": p_id, "song_id": ids["song"]["sentipiri"], "order_index": 1, "notes": "本編1曲目"},
        )
        post_data(
            "/setlist_entries/",
            {
    ...
    post_data(
        "/performance_roster/",
        {"performance_id": ids["performance"]["hokkaido"], "artist_id": ids["artist"]["unison"], "role": "Main Act"},
    )
    ...
    if user_id and ids["performance"]["hokkaido"]:
        post_data(
            "/user_attendance/",
            {"user_id": user_id, "performance_id": ids["performance"]["hokkaido"], "status": "Attended"},
        )
    ...
    if user_id and ids["album"]["jet_co"]:
        post_data(
            "/user_possessions/",
            {"user_id": user_id, "entity_type": "album", "entity_id": ids["album"]["jet_co"], "status": "Owned"},
        )
